...
static void update_model(UIState *s, const cereal::ModelDataV2::Reader &model) {
  ...
  update_line_data(s, lane_lines[1], 0, -0.05, -0.6, &scene.lane_blindspot_vertices[0], max_idx_barrier, false);
  update_line_data(s, lane_lines[2], 0, -0.05, -0.6, &scene.lane_blindspot_vertices[1], max_idx_barrier, false);
  ...
  for (int i = 0; i < std::size(scene.road_edge_vertices); i++) {
    ...
    update_line_data(s, road_edges[i], 0.1, 0, 0, &scene.road_edge_vertices[i], max_idx);
  }
  ...
  update_line_data(s, model_position, 1.0, 1.22, 1.22, &scene.track_vertices, max_idx, false);
